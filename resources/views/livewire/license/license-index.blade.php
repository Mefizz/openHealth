    ...
    <div class="max-w-7xl mx-auto">
        <table class="table-input w-full">
            ...
            @foreach($licenses as $license)
                <tr>
                    <td class="td-input">{{ Type::from($license->type)->label() }}</td>
                    <td class="td-input">{{ $license->active_from_date ?? '—' }}</td>
                    <td class="td-input">{{ $license->expiry_date ?? '—' }}</td>
                    ...
                    <td class="td-input text-center">
                        ...
                        @else
                            <div x-data="{ open: false }" class="relative inline-block text-left">
                                ...
                                <div x-show="open"
                                     x-transition
                                     class="hidden absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white dark:bg-gray-700 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                                >
                                    <div class="py-1">
                                        <a href="{{ route('license.view', [legalEntity(), $license->id]) }}"
