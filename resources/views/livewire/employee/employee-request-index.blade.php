            ...
            <div class="flex flex-col">
                <div class="flex flex-wrap items-end justify-between gap-4">
                    <div class="flex items-end gap-4">
                        <div class="w-96">
                            <x-forms.form-group>
                                <x-slot name="label"><label for="employee_search"
                    ...
                    <div class="flex items-center space-x-2 pt-5">
                        @can('create', \App\Models\Employee\EmployeeRequest::class)
                            <a href="{{ route('employee-request.create', ['legalEntity' => legalEntity()->id]) }}"
                               class="button-primary">{{ __('forms.newEmployee') }}</a>
                        @endcan
                    </div>
                </div>
                <div x-show="showFilter" x-transition class="pt-4 mt-4 border-t border-gray-200 dark:border-gray-700"
                     style="display: none;">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <x-forms.form-group>
                            <x-slot name="label"><label for="filter_phone" class="default-label">Телефон</label>
                            </x-slot>
                            <x-slot name="input">
                                <x-forms.input id="filter_phone" class="default-input" type="text"
                                               wire:model.live.debounce.300ms="filter.phone"/>
                            </x-slot>
                        </x-forms.form-group>
                        <x-forms.form-group>
                            <x-slot name="label"><label for="filter_email" class="default-label">Email</label></x-slot>
                            <x-slot name="input">
                                <x-forms.input id="filter_email" class="default-input" type="email"
                                               wire:model.live.debounce.300ms="filter.email"/>
                            </x-slot>
                        </x-forms.form-group>
                        <x-forms.form-group>
                            <x-slot name="label"><label for="filter_role" class="default-label">Роль працівника</label>
                            </x-slot>
                            <x-slot name="input">
                        ...
                        <x-forms.form-group>
                            <x-slot name="label"><label for="filter_position" class="default-label">Посада</label>
                            </x-slot>
                            <x-slot name="input">
                                <x-forms.select id="filter_position" class="default-input"
                                                wire:model.live="filter.position">
                                    <x-slot name="option">
                        ...
                        <x-forms.form-group>
                            <x-slot name="label"><label for="filter_status" class="default-label">Статус у
                                    системі</label>
                            </x-slot>
                            <x-slot name="input">
                    ...
                    <div class="flex justify-end mt-4">
                        <button type="button" wire:click="resetFilters" class="text-sm text-blue-600 hover:underline">
                            Скинути всі фільтри
                        </button>
                    </div>
                </div>
                                                    ...
                                                    @can('view', $position)
                                                        <li>
                                                            <a href="{{ route('employee-request.show', ['legalEntity' => legalEntity()->id, 'employee_request' => $position]) }}"
                                                               class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600">{{ __('forms.view') }}</a></li>
                                                    @endcan
                                                    @can('update', $position)
                                                        <li>
                                                            <a href="{{ route('employee-request.edit', ['legalEntity' => legalEntity()->id, 'employee_request' => $position]) }}"
                                                               class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600">{{ __('forms.edit') }}</a></li>
                                                    @endcan
                                                </ul>
                                                @if(!$position->uuid)
                                                    @can('delete', $position)
                                                        <div class="py-1">
                                                            <button type="button" @click="open = false"
                                                                    ...
                                                                    class="block w-full text-right py-2 px-4 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-600">{{ __('forms.delete') }}</button>
                                                        </div>
                                                    @endcan
                                                @endif
                                            </div>
