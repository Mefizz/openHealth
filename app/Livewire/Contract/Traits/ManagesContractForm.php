...
trait ManagesContractForm
{
    ...
    /**
     * Головний метод для відправки. Тепер використовує SignatureService.
     */
    public function sendApiRequest()
    {
        try {
            // 1. Валідуємо основну форму та поля для підпису
            $this->form->rulesForModelValidate();
            $this->form->validate($this->form->rulesForKepOnly()); // Припускаємо, що цей метод є у вашому Form Object

            // 2. Будуємо тіло запиту
            $payload = $this->buildPayload();
            $payload = removeEmptyKeys($payload);

            // 3. Отримуємо Tax ID власника
            ...
            // 4. ДЕЛЕГУЄМО ПІДПИСАННЯ ВАШОМУ СЕРВІСУ
            // Laravel автоматично створить екземпляр SignatureService через service container
            ...
            // 5. Відправляємо запит до eHealth
            $data = [
            ...
            // 6. Зберігаємо контракт у локальну БД
            ...
            // 7. Очищуємо кеш і перенаправляємо
            ...
            // Ваш SignatureService вже генерує ValidationException з гарними повідомленнями.
            // Livewire автоматично перехопить цю помилку і покаже користувачу.
            // Нам не потрібно нічого додатково робити.
            throw $e;
        }
    }

    /**
     * Приватний метод для підготовки даних для API.
     * Аналог вашого requestBuilder().
     */
    private function buildPayload(): array
    {
        ...
        // Видаляємо з пейлоаду поля, пов'язані з КЕП, бо вони не потрібні в eHealth
    ...
    /**
     * Ініціалізує кеш-ключ, який знадобиться для відправки запиту до API.
     */
    public function initializeCacheKey(): void
    {
        if (isset($this->legalEntity)) {
            // 'register_contract_form' - це префікс, який використовувався у вашому старому коді
